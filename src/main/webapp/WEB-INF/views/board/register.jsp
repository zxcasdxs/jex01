...
<script>
  ...
  document.querySelector("#submitBtn").addEventListener("click", (e) => {
    ...
    const fileDivArr = uploadResultDiv.querySelectorAll("div")//첨부파일마다 div생성되므로, div의 갯수가 파일의 갯수
    ...
    for(let i = 0; i < fileDivArr.length; i++){
      ...
      str += `<input type='hidden' name='file[\${i}].uuid' value='\${uuid}'>`
      str += `<input type='hidden' name='file[\${i}].fileName' value='\${fileName}'>`
      str += `<input type='hidden' name='file[\${i}].uploadPath' value='\${uploadPath}'>`
      str += `<input type='hidden' name='file[\${i}].image' value='\${image}'>`
